...
export default function UrlShortener({ user }: UrlShortenerProps) {
  ...
  const onSubmit = async (data: FormData) => {
    posthog.capture("url_shorten_attempt", {
      has_custom_code: Boolean(data.customCode && data.customCode.length > 0),
    });
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      posthog.capture("url_shorten_failed");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const copyToClipboard = async () => {
    if (result) {
      ...
      alert("URL kopiert!");
      posthog.capture("copy_short_url", { short_code: result.shortCode });
    }
  };
